...
LINEAR_AXIS 	= None			# FIXME -- You need to choose an integer value for this
ANGULAR_AXIS 	= None			# FIXME -- You need to choose an integer value for this
# -------------------------------------


# This is a "class" for a data structure.
# Right now we just initialize everything to 0.0
class make_joy_info():
	def __init__(self):
		# Set self.joy_info
		self.linear 	= 0.0	# <-- You'll see this below as self.joy_info.linear
		self.angular	= 0.0	# <-- You'll see this below as self.joy_info.linear
...
class runJoystick():
	def __init__(self):
		...
		'''
		# FIXME
		You need to define this publisher.  A few hints:
		1)  The name of the topic should be "joy_twist".
		2)  The message type should be Twist (note that we imported this above).
		'''		
		...
		while not rospy.is_shutdown():	
			...
			for event in pygame.event.get(): 
				...
				'''
				# FIXME:
				You'll need to publish a Twist message based on
				the joystick actions identified below.
				Some of these actions won't be triggered by the trackpad.
				'''
				...
				if (event.type == pygame.JOYBUTTONDOWN):
					...
					if (event.axis == LINEAR_AXIS):
						# event.value will be in the range [-1,1].
						# FIXME:  You'll have to figure out which way is forward.
						
						self.joy_info.linear = event.value	
						...
						# event.value will be in the range [-1,1].
						# FIXME:  You'll have to figure out which way is left.
					...
					if (pubTwist):
						# Publish a message to the joy_twist topic
						'''
						# FIXME
						pubTwist is True if there's something to publish.
						You've stored values for self.joy_info.linear and 
						self.joy_info.angular.
						Now it's time to publish that information.
						'''
